<%@page import="SportyShoes.com.*"%>
<%@page import="java.util.List"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>SportyShoes.com</title>
<style>
	.div-1 {
        background-color: yellow;
        width: 100%;
    }
    .div-2 {        
        width: 100%;
    }
    .div-3 {
    	position: relative;
        bottom: 0px;
        height:100px;
        width: 100%;
        background-color: grey;
    }
</style>

</head>
<body>
<% AdminLoginDetail AdDetail=new AdminLoginDetail(); %>
<div class="div-1" style="height:150px;">
	<h1><a href="AdminHome.jsp"><p style="color:blue;">SportyShoes</p></a></h1><br>
	<table align='right'><tr><td style='text-align: center'>
	<a href="AdminProfile"><p style="color:blue;"><h2><%= AdDetail.getAdName()%></h2></p></a>
	<a href=AdLogout><input type="button" size='5' value="Logout"></a></td></tr></table>
	<table align='center'><td style='text-align: center'>
	<h2>Shoes List</h2>
	</td></table>	
</div>
<div class="div-2">
	<% List<Product> list=(List<Product>)request.getAttribute("list"); %>
<table border="1px" align='center' cellspacing='5px'>
	<tr align="center"><td>Product ID</td><td>Product Name</td><td>Category</td><td>Gender</td><td>Size</td><td>Price</td><td>Stock</td><td>Update</td><td>Delete</td></tr>	
	<%for(Product pr:list){ %>
		<tr style='text-align: center'><td>
			<%= pr.getProdID() %></td><td>
			<%= pr.getProdName() %></td><td>
			<%= pr.getProdCat() %></td><td>
			<%= pr.getProdGender() %></td><td>
			<%for(ShoesSize sh:pr.getShoeSizes()){%>
			<%= sh.getShoeSize()%><br><%} %></td><td>
			<%for(ShoesSize sh:pr.getShoeSizes()){%>
			<%= sh.getProdPrice()%>Rs.<br><%} %></td><td>
			<%for(ShoesSize sh:pr.getShoeSizes()){%>
			<%= sh.getStock()%><br><%} %></td><td>
			<a href="UpdateShoesFromList?ProdID=<%=pr.getProdID() %>"><p style="color:blue;font-size:15px;">Update</p></a></td><td>
			<a href="DeleteShoesFromList?ProdID=<%=pr.getProdID() %>"><p style="color:blue;font-size:15px;">Delete</p></a>
	</td></tr>
	<% } %>
</table>
</div>
<div class="div-3">
	<table align="left"><td style='text-align: center' width='180 px'>
	<h2>Developed By:<br>Rahul Ojha</h2>
	</td></table>	
	
</div>
</body>
</html>